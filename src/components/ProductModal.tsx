...
export const ProductModal: React.FC<ProductModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/80 backdrop-blur-md animate-fade-in overflow-y-auto">
      <div 
        className="relative w-full max-w-3xl bg-[#141414] rounded-3xl shadow-2xl border border-rose-900/40 overflow-hidden my-auto max-h-[90vh] flex flex-col md:flex-row text-slate-200"
        ...
        <div className="w-full md:w-1/2 p-5 sm:p-7 flex flex-col justify-between overflow-y-auto">
          <div>
            ...
            <div className="flex items-center justify-between text-xs mb-2">
              <span className="px-2.5 py-0.5 rounded-md bg-rose-950/50 text-rose-300 font-mono font-medium border border-rose-900/30">
                كود المنتج: {product.code || 'ROSA-01'}
              </span>
            ...
            <h3 className="text-xl sm:text-2xl font-bold text-slate-100 mb-2 leading-snug">
              {product.title}
            </h3>
            ...
            {product.material && (
              <div className="mb-4 text-xs text-zinc-300 bg-[#1a1a1a] p-2.5 rounded-xl border border-rose-900/20">
                <span className="font-bold text-rose-300">المواد والتصنيع: </span>
                <span>{product.material}</span>
              </div>
            )}

            {/* Quantity Selector */}
            <div className="flex items-center justify-between py-3 border-y border-rose-900/30 mb-4">
              <span className="text-xs sm:text-sm font-bold text-slate-200">الكمية المطلوب طلبها:</span>
              <div className="flex items-center gap-3 bg-[#1b1417] rounded-full p-1 border border-rose-900/40">
                <button
                  onClick={() => setQuantity(Math.max(1, quantity - 1))}
                  className="w-8 h-8 rounded-full bg-[#241a1f] text-rose-300 hover:bg-rose-900/60 flex items-center justify-center font-bold shadow-2xs transition-all border border-rose-800/30"
                >
                  <Minus className="w-3.5 h-3.5" />
                </button>
                <span className="w-8 text-center text-sm font-extrabold text-rose-200">{quantity}</span>
                <button
                  onClick={() => setQuantity(quantity + 1)}
                  className="w-8 h-8 rounded-full bg-rose-800 text-white hover:bg-rose-700 flex items-center justify-center font-bold shadow-2xs transition-all"
                >
            ...
            {/* Optional Customer Personalization for WhatsApp */}
            <div className="space-y-2 mb-4 bg-[#191114] p-3 rounded-2xl border border-rose-900/30">
              <p className="text-[11px] font-bold text-rose-300 mb-1">
                تأكيد بياناتك لتسهيل تجهيز الطلب بالواتساب (اختياري):
              </p>
          ...
          <div className="pt-2 space-y-2.5">
            ...
            <a
              ...
              className="w-full py-3.5 px-4 rounded-2xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm sm:text-base flex items-center justify-center gap-2 transition-all shadow-lg shadow-emerald-950/50 hover:scale-101"
            >
              <MessageCircle className="w-5 h-5 fill-current" />
              <span>إرسال الطلب الآن عبر الواتساب</span>
            </a>
            ...
            <button
              ...
              className="w-full py-3 px-4 rounded-2xl bg-rose-950/40 hover:bg-rose-900/60 text-rose-200 font-bold text-xs sm:text-sm flex items-center justify-center gap-2 transition-all border border-rose-800/40 cursor-pointer"
            >
